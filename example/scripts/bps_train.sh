...
do
	...
	command="$SCRIPT_PATH/bps_scheduler.sh $DMLC_NUM_WORKER $DMLC_NUM_SERVER $DMLC_PS_ROOT_URI $DMLC_PS_ROOT_PORT $OUTPUT_FILE"
	$SSH $SLAVE $command

	OUTPUT_FILE="/mnt/bps-server-${MODEL}.log"
	command="$SCRIPT_PATH/bps_server.sh $DMLC_NUM_WORKER $DMLC_NUM_SERVER $DMLC_PS_ROOT_URI $DMLC_PS_ROOT_PORT $OUTPUT_FILE"
	$SSH $SLAVE $command
done 3< $SLAVE_FILE
sleep 10
id=0
